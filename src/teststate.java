	...
	public boolean SafeInit() {
		...
		button2.addButtonCallback(new GameButtonCallback() {
			public void buttonPressed(GameButton button) {sound2.play(false);
														  ...
														  button2.setY(button2.getY()+2);
														  if(paused)
															  paused = false;
														  else 
															  paused = true;}
			public void buttonReleased(GameButton button){button2.setX(button2.getX()-2);button2.setY(button2.getY()-2);}
			public void buttonExited(GameButton button) {button2.setBackGroundColor(Color.red);}
			public void buttonEntered(GameButton button) {button2.setBackGroundColor(Color.pink);}
		});
		gui.setFont(font);
		gui.add(button);
		gui.add(button2);
